...
const startRoom = (roomID) => {
  let roomData = all_rooms[roomID]['singleplayer']
    ? generateNewRoomDataSingleplayer(roomID)
    : generateNewRoomData(roomID)
    ...
    if (
      ...
      all_rooms[roomID]['playerInfos'] = all_rooms[roomID]['playerInfos'].map(
        (playerInfo) => ({
          ...playerInfo,
          reMatch:
            (playerInfo['name'] == 'NoobBot' ||
              playerInfo['name'] == 'DoomBot') &&
            all_rooms[roomID]['singleplayer']
              ? true
              : false,
        })
      )
